...
- **빈(신규) 저장소** — 커밋이 하나도 없는 repo 에서 첫 step 커밋 시 `git reset HEAD -- phases` 가 unborn HEAD 를 못 풀어 하네스가 **loud abort** 로 중단된다(데이터 손상 아님, 재현 가능). **커밋이 1개 이상 있는 repo 에서 실행**하면 발생하지 않으므로 그렇게 쓴다(빈 repo 지원이 실제로 필요해지면 그때 unborn-HEAD 가드를 추가 — YAGNI).
